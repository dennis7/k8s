...
kubectl apply -f k8s 
kubectl set image deployments/client-deployment client=dennis07/multi-client:$SHA 
kubectl set image deployments/server-deployment server=dennis07/multi-server:$SHA 
kubectl set image deployments/worker-deployment worker=dennis07/multi-worker:$SHA 
